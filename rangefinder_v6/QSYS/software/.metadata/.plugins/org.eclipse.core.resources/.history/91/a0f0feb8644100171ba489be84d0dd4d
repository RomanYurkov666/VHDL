...
void rs485_routine(t_rs485_config *c)
{
	...
	{
		for (i=0;i<c->rx_byte_cnt-PC_MIN_MSG_SIZE;i++) //ѕросматриваем каждый байт с первого и до последнего, который в принципе удален от конца настолько, что может быть началом целого пакета
		{
			if (c->rx_buf[i] == SIGN) //≈сли текущий байт совпал со стартовой сигнатурой
			{
				...
				if (pack_len) //≈сли сообщение целое, то передаем его обработчику
				{
